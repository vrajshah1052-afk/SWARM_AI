          ...
          <Panel dense>
            <div className="relative overflow-hidden rounded-lg border border-white/[0.09] bg-ink-950">
              ...
              <div className="pointer-events-none absolute right-2.5 bottom-2.5 rounded-md border border-white/10 bg-ink-950/75 px-2.5 py-1.5 font-mono text-[10px] text-slate-400 backdrop-blur">
                Σ {frame?.stats.collected ?? 0} delivered ·{" "}
                {(frame?.stats.coverage ?? 0) * 100 > 0
                  ? ((frame?.stats.coverage ?? 0) * 100).toFixed(1)
                  : "0.0"}
                % coverage
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <Btn
                onClick={() => setIdx((i) => Math.min((rec?.frames.length ?? 1) - 1, i + 1))}
                disabled={!rec}
              >
                Next ▶
              </Btn>
            ...
            <div className="mt-1 flex justify-between font-mono text-[10px] text-slate-600">
              <span>00:00</span>
              <span>{rec ? `${(((idx + 1) / rec.frames.length) * 100).toFixed(0)}%` : "—"}</span>
          ...
          <Panel title="Frame inspector">
            {frame ? (
              <div className="grid grid-cols-2 gap-2">
                ...
                <Stat
                  label="avg trip"
                  value={formatNum(frame.stats.avgTrip)}
                  unit="t"
                  accent="violet"
                />
                ...
                <Stat
                  label="uplink"
                  value={frame.stats.bandwidthKbps.toFixed(1)}
                  unit="kbps"
                  accent="cyan"
                />
              </div>
