...
const PARAMS: [string, string, string, string][] = [
  [
    ...
    "int",
    "20 – 900",
    "Fleet size. Cost scales linearly; trail competition emerges above ~500.",
  ],
  [
    "speed",
    "float",
    "0.3 – 2.0",
    "Cells travelled per tick. High speed under-samples the field and blurs trails.",
  ],
  [
    "evaporation",
    "float λ",
    "0.001 – 0.05",
    "Fraction of every cell's intensity removed per tick. Sets the memory horizon ≈ 1/λ ticks.",
  ],
  [
    "diffusion",
    "float D",
    "0 – 0.6",
    "Blend factor toward the 3×3 mean. Widens the sensing basin, softens gradients.",
  ],
  [
    "deposit",
    "float δ",
    "4 – 80",
    "Trace written per agent per tick, attenuated by e^(−age/τ) with τ = 340.",
  ],
  ...
  [
    ...
    "cells",
    "2 – 16",
    "Whisker reach. Long whiskers create smoother, wider highways.",
  ],
  ...
  [
    ...
    "float w",
    "0 – 1",
    "Uniform noise added to heading each tick — the exploration pressure.",
  ],
  [
    "foodClusters",
    "int",
    "1 – 9",
    "Number of radial resource deposits scattered 34–86 cells from the nest.",
  ],
      ...
      <div className="grid gap-8 lg:grid-cols-[210px_minmax(0,1fr)_220px]">
        ...
        <article className="min-w-0 space-y-12 pb-16">
          <Section id="overview" title="Overview">
            <P>
              <B>Digital Pheromone</B> is a browser-native research engine for stigmergic
              multi-robot coordination. Agents never exchange plans, maps or goals. They write
              scalar traces into a shared lattice and read the traces left by others — a mechanism
              biologists call <em>stigmergy</em>, first described in termite nest construction by
              Grassé in 1959.
            </P>
            <P>
              Two independent fields are maintained. <Code>Φ_home</Code> is deposited by searching
              agents and forms a gradient back to the nest. <Code>Φ_food</Code> is deposited by
              loaded carriers and recruits others to a resource. The interaction of deposition,
              diffusion and evaporation performs a distributed, anytime shortest-path computation —
              with no global state and no consensus protocol.
            </P>
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                [
                  "No map",
                  "The environment is the memory. Robots are stateless apart from a carry flag.",
                ],
...
for (const robots of [80, 240, 480, 800]) {
  ...
  console.log(robots, engine.stats.collected, engine.stats.bandwidthKbps);
          ...
          <Section id="physics" title="Pheromone physics">
            <P>
              Each field evolves under an explicit Euler integration of a reaction–diffusion
              equation:
            </P>
            ...
            <ul className="ml-4 list-disc space-y-2 text-[13px] leading-relaxed text-slate-400 marker:text-cyan-500">
              ...
              <li>
                ...
                widens the attraction basin so a robot can detect a trail it never physically
                crossed.
              </li>
              <li>
            ...
            <Callout tone="cyan" title="Why trails shorten over time">
              Two agents leaving a resource simultaneously reach the nest at different times. The
              one that took the shorter route reinforces its path sooner and with less age
              attenuation, so the shorter branch accrues intensity faster. Evaporation removes the
              loser. Convergence is positive feedback plus forgetting — nothing else.
            </Callout>
          </Section>

          <Section id="behaviours" title="Robot behaviours">
            <P>
              Every agent is a finite-state machine with exactly two states and no memory of the
              world beyond its own heading.
            </P>
...
pos = clamp(pos + speed · û(a));      // walls reflect or slide`}</Pre>
          ...
          <Section id="bandwidth" title="Bandwidth model">
            <P>
              Real swarms are bounded by radio, not compute. The engine estimates the uplink
              required to keep a shared field coherent across the fleet.
            </P>
...
compression  = naiveKbps / kbps`}</Pre>
            <div className="grid gap-3 sm:grid-cols-2">
              <Panel title="Why it compresses">
                <ul className="ml-4 list-disc space-y-1.5 text-[12px] text-slate-400 marker:text-cyan-500">
                  ...
                  <li>
                    Values are logarithmically perceived by the controller — precision is cheap to
                    lose.
                  </li>
                </ul>
              </Panel>
            ...
            <Callout tone="amber" title="Practical floor">
              Below 2 bits per cell the gradient quantises into plateaus and whisker comparisons
              start tying, which the controller resolves as random turns. Yield collapses roughly
              30–45%. Run the{" "}
          ...
          <Section id="api" title="API reference">
            ...
            />
            <ApiBlock
              sig="engine.step(): void"
              desc="Advances one tick: evaporate → diffuse → move & deposit → interact → accumulate statistics (every 6 ticks) and history (every 12)."
            />
            <ApiBlock
              sig="engine.setParams(p: Partial<Params>): void"
              desc="Hot-swaps parameters mid-run. Changing `robots` adds or removes agents without resetting the field."
            />
            <ApiBlock
              sig="engine.reset(seed?: number): void"
              desc="Clears both fields, regenerates obstacles and resources, and re-spawns the fleet. Pass a seed for reproducibility."
            />
            <ApiBlock
              sig="engine.snapshot(): Frame"
              desc="Returns a compact, structured-cloneable frame: ≤500 quantised agent poses plus 100 × 63 uint8 copies of both fields."
            />
            <ApiBlock
              sig="runHeadless(params, ticks, every, seed?)"
              desc="Runs a full experiment with no renderer attached and returns { engine, frames }."
            />
            <ApiBlock
              sig="renderEngine(canvas, engine, view)"
              desc="Paints an ImageData heatmap at lattice resolution, upscales it, then overlays the nest glow and agents."
            />
            <ApiBlock
              sig="renderIso(canvas, frame, rotate, height)"
              desc="Isometric bar-field projection of a captured frame — the dashboard's pseudo-3D relief view."
            />
...
type Recording = {
  id, name, createdAt, params, frames, foodSites, nest, history
            ...
            <P>
              ...
              about 2 MB in memory. Recordings are session-scoped by design — export the JSON
              summary if you need to keep results.
            </P>
          </Section>
            ...
            {[
              ...
              >
                <summary className="cursor-pointer list-none text-[13px] font-semibold text-slate-100 marker:hidden">
                  <span className="mr-2 text-cyan-400 transition group-open:rotate-90 inline-block">
                    ▸
                  </span>
                  {q}
                </summary>
          ...
          <div className="sticky top-20 space-y-4">
            ...
            <Panel title="Further reading" dense>
              <ul className="space-y-2 text-[11.5px] text-slate-400">
                <li>
                  Grassé (1959) —{" "}
                  <span className="text-slate-500">La théorie de la stigmergie</span>
                </li>
                <li>
                ...
                <li>
                  Dorigo &amp; Stützle (2004) —{" "}
                  <span className="text-slate-500">Ant Colony Optimization</span>
                </li>
                <li>
                  Payton et al. (2001) — <span className="text-slate-500">Pheromone robotics</span>
                </li>
              </ul>
            </Panel>
...
function Code({ children }: { children: ReactNode }) {
  return (
    <code className="rounded bg-white/[0.07] px-1.5 py-0.5 font-mono text-[12px] text-cyan-300">
      {children}
    </code>
  );
}
...
function StateCard({ title, color, lines }: { title: string; color: string; lines: string[] }) {
  return (
    <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4">
      <div className={cn("font-mono text-[12px] font-semibold tracking-widest", color)}>
        {title}
      </div>
